...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        '''
        # Count the frequency of each class in the training labels
        class_counts = Counter(Y)
        # Get the most frequent class
        self.mostFrequentClass = class_counts.most_common(1)[0][0]
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        '''
        # Separate data based on first feature value
        pos_indices = X[:, 0] > 0
        neg_indices = X[:, 0] <= 0
        
        # Find most frequent class for positive first feature
        if any(pos_indices):
            pos_labels = Y[pos_indices]
            pos_class_counts = Counter(pos_labels)
            self.classForPos = pos_class_counts.most_common(1)[0][0]
        
        # Find most frequent class for negative first feature
        if any(neg_indices):
            neg_labels = Y[neg_indices]
            neg_class_counts = Counter(neg_labels)
            self.classForNeg = neg_class_counts.most_common(1)[0][0]
